...
_src/test/java/it/megadix/orientdb/spring/samples/SamplesTestConfiguration.java_

To make it work:
* install Maven 3.x+
* startup OrientDB instance with _demo_ database;
* launch this test: it.megadix.orientdb.spring.SampleServiceTest

mvn test -Dtest=SampleServiceTest

An IDE such Eclipse or Netbeans is higly recommended, so you can debug things.
